class Serializable(dict):
    """
    Intended for json.dumps() compatibility, by inheriting this class.

    Copied from (with minor tweak):
    ...
    def _myattrs(self):
        return [
            ...
            if x not in Serializable().__dir__() and not hasattr(getattr(self, x), '__call__')  # omit functions
        ]
